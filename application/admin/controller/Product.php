...
class Product extends Backend
{
    ...
    {
        $row = ['name'=>'', 'exa_image'=>'', 'id'=>''];
        if($ids){
            $row = $this->tempModel->get($ids);
            if (!$row) {
                
                $this->error(__('No Results were found'));
            }
            if ($row['is_shelves']==1) {
                $this->error(__('该模板已上架'));
            }
        }
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    $this->tempModel->where('id', $params['temp_id'])->update(['is_shelves'=>1]);
                    Db::commit();
    ...
    public function del($ids = "")
    {
        ...
        if ($ids) {
            ...
            try {
                foreach ($list as $k => $v) {
                    $count += $v->delete();
                    $this->tempModel->where('id', $v['temp_id'])->update(['is_shelves'=>0]);
                }
                Db::commit();
            } catch (PDOException $e) {
                Db::rollback();
